...
class PlannerDownloaderTest(unittest.TestCase):
    ...
    def test_parallel_download_logs_geogetter_error_and_continues(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            output_dir = root / "out"
            data = b"@r2\nTGCA\n+\n!!!!\n"
            source = root / "fixture2.fastq.gz"
            source.write_bytes(data)
            fastq1 = FastqFile(
                source_accession="FIXTURE",
                query_accession="FIXTURE",
                run_accession="FIXTURE_RUN_1",
                file_index=1,
                file_name="fixture1.fastq.gz",
            ...
            fastq2 = FastqFile(
                source_accession="FIXTURE",
                query_accession="FIXTURE",
                run_accession="FIXTURE_RUN_2",
                file_index=1,
                file_name="fixture2.fastq.gz",
                url=source.as_uri(),
                expected_md5=hashlib.md5(data).hexdigest(),
                size_bytes=len(data),
            )
            plan = build_download_plan("FIXTURE", "FIXTURE", [fastq1, fastq2], output_dir)
            (output_dir / "fixture1.fastq.gz").mkdir()

            results = download_plan(plan, download_workers=2)

            self.assertEqual([status for _planned, status, _message in results], ["output_path_invalid", "md5_verified"])
            log_text = download_log_path(output_dir).read_text(encoding="utf-8-sig")
            self.assertIn("output_path_invalid", log_text)
            self.assertIn("md5_verified", log_text)
            self.assertEqual((output_dir / "fixture2.fastq.gz").read_bytes(), data)

    def test_parallel_download_logs_unexpected_exception_and_continues(self):
        with tempfile.TemporaryDirectory() as temp:
            output_dir = Path(temp) / "out"
            data_by_name = {
                "fixture1.fastq.gz": b"@r1\nACGT\n+\n!!!!\n",
            ...
            def fake_download_one(planned, progress_callback=None, message_callback=None):
                if planned.fastq.file_name == "fixture1.fastq.gz":
            ...
            with mock.patch("geo_getter.downloader.download_one", side_effect=fake_download_one):
                results = download_plan(plan, download_workers=2)

            self.assertEqual([status for _planned, status, _message in results], ["internal_error", "md5_verified"])
            log_text = download_log_path(output_dir).read_text(encoding="utf-8-sig")
    ...
    def test_existing_file_without_md5_matching_size_is_reused(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            ...
            self.assertEqual(results[0][1], "md5_unavailable")
            self.assertEqual(existing.read_bytes(), b"stale data without md5\n")
            self.assertFalse(list(output_dir.glob("fixture.fastq.gz.unverified-existing-*")))
    ...
    def test_existing_directory_at_fastq_path_is_rejected_without_moving_it(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(results[0][1], "output_path_invalid")
            self.assertTrue(target.is_dir())
            with download_log_path(output_dir).open("r", encoding="utf-8-sig", newline="") as handle:
                rows = list(csv.DictReader(handle, delimiter="\t"))
            self.assertEqual(rows[-1]["status"], "output_path_invalid")
    ...
    def test_complete_part_file_without_md5_matching_size_is_reused(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            output_dir = root / "out"
            output_dir.mkdir()
            ...
            self.assertFalse(part.exists())
            self.assertEqual((output_dir / "fixture.fastq.gz").read_bytes(), data)
            self.assertFalse(list(output_dir.glob("fixture.fastq.gz.part.unverified-existing-*")))
    ...
    def test_resume_required_bytes_rechecks_same_size_existing_md5(self):
        with tempfile.TemporaryDirectory() as temp:
            output_dir = Path(temp) / "out"
            ...
            plan = build_download_plan("FIXTURE", "FIXTURE", [fastq], output_dir)
            write_fastq_outputs(plan)
            plan.files[0].local_path.write_bytes(b"bad!")

            resume = validate_resume_artifacts(plan)

            self.assertEqual(resume.required_bytes, len(expected_data))
    ...
    def test_download_rejects_short_content_length_without_expected_size(self):
        with tempfile.TemporaryDirectory() as temp:
            local_path = Path(temp) / "supplementary.txt"
            responses = [
                FakeUrlopenResponse(data=b"abc", headers={"Content-Length": "10"})
                for _index in range(4)
            ]

            def urlopen_short_body(_request, timeout):
                return responses.pop(0)

            with (
                mock.patch("geo_getter.downloader.urllib.request.urlopen", side_effect=urlopen_short_body),
                mock.patch("geo_getter.downloader.time.sleep", return_value=None),
            ):
                with self.assertRaises(DownloadNetworkError):
                    download_url_to_part("https://example.invalid/supplementary.txt", local_path)

            self.assertFalse(local_path.exists())
            self.assertEqual(local_path.with_name("supplementary.txt.part").read_bytes(), b"abc")

    def test_download_url_without_md5_rejects_short_content_length(self):
        with tempfile.TemporaryDirectory() as temp:
            local_path = Path(temp) / "supplementary.txt"
            responses = [
                FakeUrlopenResponse(data=b"abc", headers={"Content-Length": "10"})
    ...
    def test_resume_rejects_missing_or_mismatched_content_range(self):
        ...
        for content_range, content_length in cases:
    ...
    def test_single_worker_exception_is_recorded_as_file_outcome(self):
        with tempfile.TemporaryDirectory() as temp:
            output_dir = Path(temp) / "out"
            fastq = FastqFile(
                source_accession="FIXTURE",
            ...
            plan = build_download_plan("FIXTURE", "FIXTURE", [fastq], output_dir)
            plan.files[0].local_path.mkdir()

            results = download_plan(plan, download_workers=1)

            self.assertEqual(results[0][1], "output_path_invalid")
            with download_log_path(output_dir).open("r", encoding="utf-8-sig", newline="") as handle:
                rows = list(csv.DictReader(handle, delimiter="\t"))
            self.assertEqual(rows[-1]["status"], "output_path_invalid")
            self.assertEqual(rows[-1]["file_name"], "fixture.fastq.gz")
    ...
    def test_verify_fastq_manifest_rejects_relative_local_path_escape(self):
        with tempfile.TemporaryDirectory() as temp:
            output_dir = Path(temp) / "out"
            output_dir.mkdir()
            outside = Path(temp) / "outside.fastq.gz"
            ...
            manifest.write_text(
                "\n".join(
                    [
                        "source_accession\tquery_accession\trun_accession\tfile_index\tfile_name\turl\texpected_md5\tsize_bytes\tlocal_path\tstatus",
                        f"GSE\tSRP\tSRR\t1\toutside.fastq.gz\thttps://example.invalid/outside.fastq.gz\t\t{outside.stat().st_size}\t../outside.fastq.gz\tplanned",
                    ]
                ),
                encoding="utf-8-sig",
            )
            ...
            self.assertIn("local_path_escapes_manifest_dir", context.exception.detail)
